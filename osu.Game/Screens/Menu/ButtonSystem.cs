...
namespace osu.Game.Screens.Menu
{
    public partial class ButtonSystem : Container, IStateful<ButtonSystemState>, IKeyBindingHandler<GlobalAction>
    {
        ...
        private void load(AudioManager audio, IdleTracker? idleTracker, GameHost host)
        {
            ...
            //     Padding = new MarginPadding { Left = WEDGE_WIDTH },
            // });
            // since no multiplayer buttons are present for now we're just gonna redirect this to solo
            buttonsTopLevel.Add(new MainMenuButton(ButtonSystemStrings.Play, @"button-default-select", OsuIcon.Logo, new Color4(102, 68, 204, 255), (_, _) => OnSolo?.Invoke(), Key.P)
            {
                Padding = new MarginPadding { Left = WEDGE_WIDTH },
            });
